...
contract RandomExample is usingOraclize {
    ...
    function __callback(bytes32 _queryId, string _result, bytes _proof)
    {
        ...
        if (oraclize_randomDS_proofVerify__returnCode(_queryId, _result, _proof) != 0) {
            ...
            uint maxRange = 2**(8* 1); // this is the highest uint we want to get. It should never be greater than 2^(8*N), where N is the number of random bytes we had asked the datasource to return
